...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _init: function (options, refreshMode) {
            ...
            self.$progress = $cont.find('.kc-upload-progress');
            ...
            self.$errorContainer = $h.getElement(options, 'elErrorContainer',
                self.$previewContainer.find('.kc-fileinput-error'));
                ...
                init: function (id, f, index) {
                    ...
                    if (self.showPreview) {
                        ...
                        if (rm.$thumb && rm.$thumb.length) {
                            ...
                            rm.$btnDelete = rm.$thumb.find('.kc-file-remove');
                        }
                    }
        ...
        _initTemplateDefaults: function () {
            var self = this, tMain1, tMain2, tPreview, tFileIcon, tClose, tCaption, tBtnDefault, tBtnLink, tBtnBrowse,
                ...
                tStats, tModalLabel, renderObject = function (type, mime) {
                    return '<object class="kc-preview-data file-preview-' + type + '" title="{caption}" ' +
                        'data="{data}" type="' + mime + '"' + tStyle + '>\n' + $h.DEFAULT_PREVIEW + '\n</object>\n';
                };
            tMain1 = '{preview}\n' +
                '<div class="kc-upload-progress kc-hidden"></div><div class="clearfix"></div>\n' +
                ...
                '    </div>\n' +
                '</div>';
            tMain2 = '{preview}\n<div class="kc-upload-progress kc-hidden"></div>\n<div class="clearfix"></div>\n' +
                '{remove}\n{cancel}\n{upload}\n{browse}\n';
            tPreview = '<div class="file-preview {class}">\n' +
                ...
                '    <div class="kc-fileinput-error"></div>\n' +
                '  </div>\n' +
                '</div>';
            ...
            tModal = '<div class="modal-dialog modal-lg{rtl}" role="document">\n' +
                ...
                '      <span class="kc-zoom-title"></span>\n' +
                '      <div class="kc-zoom-actions">{toggleheader}{fullscreen}{borderless}{close}</div>\n' +
                '    </div>\n' +
                '    <div class="modal-body">\n' +
                '      <div class="floating-buttons"></div>\n' +
                '      <div class="kc-zoom-body file-zoom-content {zoomFrameClass}"></div>\n' + '{prev} {next}\n' +
                '    </div>\n' +
                '  </div>\n' +
                '</div>\n';
            ...
            tActions = '<div class="file-actions">\n' +
                ...
                '<div class="clearfix"></div>';
            //noinspection HtmlUnknownAttribute
            tActionDelete = '<button type="button" class="kc-file-remove {removeClass}" ' +
                'title="{removeTitle}" {dataUrl}{dataKey}>{removeIcon}</button>\n';
            tActionUpload = '<button type="button" class="kc-file-upload {uploadClass}" title="{uploadTitle}">' +
                '{uploadIcon}</button>';
            tActionDownload = '<a class="kc-file-download {downloadClass}" title="{downloadTitle}" ' +
                'href="{downloadUrl}" download="{caption}" target="_blank">{downloadIcon}</a>';
            tActionZoom = '<button type="button" class="kc-file-zoom {zoomClass}" ' +
                'title="{zoomTitle}">{zoomIcon}</button>';
            tActionDrag = '<span class="file-drag-handle {dragClass}" title="{dragTitle}">{dragIcon}</span>';
            tIndicator = '<div class="file-upload-indicator" title="{indicatorTitle}">{indicator}</div>';
            tTagBef = '<div class="file-preview-frame {frameClass}" id="{previewId}" data-fileindex="{fileindex}"' +
                ' data-fileid="{fileid}" data-template="{template}"';
            tTagBef1 = tTagBef + '><div class="kc-file-content">\n';
            tTagBef2 = tTagBef + ' title="{caption}"><div class="kc-file-content">\n';
            tTagAft = '</div>{footer}\n{zoomCache}</div>\n';
            tGeneric = '{content}\n';
            tStyle = ' {style}';
            ...
            tImage = '<img src="{data}" class="file-preview-image kc-preview-data" title="{title}" alt="{alt}"' +
                tStyle + '>\n';
            tOffice = '<iframe class="kc-preview-data file-preview-office" ' +
                'src="https://view.officeapps.live.com/op/embed.aspx?src={data}"' + tStyle + '></iframe>';
            tGdocs = '<iframe class="kc-preview-data file-preview-gdocs" ' +
                'src="https://docs.google.com/gview?url={data}&embedded=true"' + tStyle + '></iframe>';
            tVideo = '<video class="kc-preview-data file-preview-video" controls' + tStyle + '>\n' +
                '<source src="{data}" type="{type}">\n' + $h.DEFAULT_PREVIEW + '\n</video>\n';
            tAudio = '<!--suppress ALL --><audio class="kc-preview-data file-preview-audio" controls' + tStyle + '>\n<source src="{data}" ' +
                'type="{type}">\n' + $h.DEFAULT_PREVIEW + '\n</audio>\n';
            tFlash = '<embed class="kc-preview-data file-preview-flash" src="{data}" type="application/x-shockwave-flash"' + tStyle + '>\n';
            tObject = '<object class="kc-preview-data file-preview-object file-object {typeCss}" ' +
                'data="{data}" type="{type}"' + tStyle + '>\n' + '<param name="movie" value="{caption}" />\n' +
                $h.OBJECT_PARAMS + ' ' + $h.DEFAULT_PREVIEW + '\n</object>\n';
            tOther = '<div class="kc-preview-data file-preview-other-frame"' + tStyle + '>\n' + $h.DEFAULT_PREVIEW + '\n</div>\n';
            tZoomCache = '<div class="kc-zoom-cache">{zoomContent}</div>';
            ...
            self.defaults = {
                ...
                fileActionSettings: {
                    ...
                    removeClass: 'btn btn-sm btn-kc btn-default btn-outline-secondary',
                    removeErrorClass: 'btn btn-sm btn-kc btn-danger',
                    removeTitle: 'Remove file',
                    uploadIcon: '<i class="glyphicon glyphicon-upload"></i>',
                    uploadClass: 'btn btn-sm btn-kc btn-default btn-outline-secondary',
                    uploadTitle: 'Upload file',
                    uploadRetryIcon: '<i class="glyphicon glyphicon-repeat"></i>',
                    uploadRetryTitle: 'Retry upload',
                    downloadIcon: '<i class="glyphicon glyphicon-download"></i>',
                    downloadClass: 'btn btn-sm btn-kc btn-default btn-outline-secondary',
                    downloadTitle: 'Download file',
                    zoomIcon: '<i class="glyphicon glyphicon-zoom-in"></i>',
                    zoomClass: 'btn btn-sm btn-kc btn-default btn-outline-secondary',
            ...
            $.each(self.previewContentTemplates, function (key, value) {
                if ($h.isEmpty(self.previewTemplates[key])) {
                    ...
                    if (self._isPdfRendered() && key === 'pdf') {
                        tagBef = tagBef.replace('kc-file-content', 'kc-file-content kc-pdf-rendered');
                    }
        ...
        _errorsExist: function () {
            ...
            $err.find('.kc-error-close').remove();
            $err.find('ul').remove();
        ...
        _addError: function (msg) {
            ...
            if (msg && $error.length) {
                ...
                self._handler($error.find('.kc-error-close'), 'click', function () {
                    setTimeout(function () {
        ...
        _autoFitContent: function () {
            ...
            $.each(config, function (cat, settings) {
                ...
                self.$preview.find(sel + '.kc-preview-data,' + sel + ' .kc-preview-data').css(settings);
            });
        },
        ...
        _initPreviewImageOrientations: function () {
            ...
            self.getFrames('.file-preview-initial').each(function () {
                ...
                if (config && config.exif && config.exif.Orientation) {
                    id = $thumb.attr('id');
                    $img = $thumb.find('>.kc-file-content img');
                    $zoomImg = self._getZoom(id, ' >.kc-file-content img');
                    if (canOrientImage) {
        ...
        _maximizeZoomDialog: function () {
            ...
            $modal.find('.kc-zoom-body').height(h);
        },
            ...
            if ($modal.hasClass('file-zoom-fullscreen')) {
                ...
                if (!fullScreen) {
                    if (!$btnFull.hasClass('active')) {
                        ...
                        self.$modal.find('.kc-zoom-body').css('height', self.zoomModalHeight);
                    } else {
                        $btnFull.removeClass('active').attr('aria-pressed', 'false');
                    }
                } else {
        ...
        _setZoomContent: function ($frame, animate) {
            ...
            $content = $zoomPreview.find('.kc-file-content');
            body = $content.length ? $content.html() : '';
            cap = $frame.data('caption') || '';
            size = $frame.data('size') || '';
            title = cap + ' ' + size;
            $modal.find('.kc-zoom-title').attr('title', $('<div/>').html(title).text()).html(title);
            $body = $modal.find('.kc-zoom-body');
            $modal.removeClass('kc-single-content');
            if (animate) {
            ...
            if (config) {
                $dataEl = $body.find('.kc-preview-data');
            ...
            self._handler($btnTogh, 'click', function () {
                var $header = $modal.find('.modal-header'), $floatBar = $modal.find('.modal-body .floating-buttons'),
                    ht, $actions = $header.find('.kc-zoom-actions'), resize = function (height) {
                        var $body = self.$modal.find('.kc-zoom-body'), h = self.zoomModalHeight;
                        if ($modal.hasClass('file-zoom-fullscreen')) {
        ...
        _zoomSlideShow: function (dir, previewId) {
            var self = this, $btn = self.$modal.find('.kc-zoom-actions .btn-' + dir), $targFrame, i, $thumb,
            ...
            for (i = 0; i < len; i++) {
                $thumb = $(thumbsData[i]);
                if ($thumb && $thumb.length && $thumb.find('.kc-file-zoom:visible').length) {
        ...
        _initZoomButton: function () {
            var self = this;
            self.$preview.find('.kc-file-zoom').each(function () {
                var $el = $(this);
            ...
            if (out.initialPreview !== undefined && out.initialPreview.length > 0) {
                ...
                if ($thumb !== undefined) {
                    if (!allFiles) {
                        ...
                        $newCache = $div.find('.kc-zoom-cache');
        ...
        _initSuccessThumbs: function () {
            ...
            self._getThumbs($h.FRAMES + '.file-preview-success').each(function () {
                var $thumb = $(this), $remove = $thumb.find('.kc-file-remove');
        ...
        _uploadSingle: function (i, id, isBatch) {
            ...
            if (self.showPreview) {
                ...
                $btnUpload = $thumb.find('.kc-file-upload');
                $btnDelete = $thumb.find('.kc-file-remove');
                $prog.show();
            }
        ...
        _uploadBatch: function () {
            ...
            fnBefore = function (jqXHR) {
                ...
                if (self.showPreview) {
                    self._getThumbs().each(function () {
                        var $thumb = $(this), $btnUpload = $thumb.find('.kc-file-upload'),
                            $btnDelete = $thumb.find('.kc-file-remove');
                        if (!$thumb.hasClass('file-preview-success')) {
                            self._setThumbStatus($thumb, 'Loading');
                            $h.addCss($thumb, 'file-uploading');
                ...
                if ($h.isEmpty(data) || $h.isEmpty(data.error)) {
                    ...
                    if (self.showPreview) {
                        $thumbs.each(function () {
                            ...
                            self._setThumbStatus($thumb, 'Success');
                            $thumb.removeClass('file-uploading');
                            $thumb.find('.kc-file-upload').hide().removeAttr('disabled');
                        });
                    ...
                    if (self.showPreview) {
                        $thumbs.each(function () {
                            var $thumb = $(this);
                            $thumb.removeClass('file-uploading');
                            $thumb.find('.kc-file-upload').removeAttr('disabled');
                            $thumb.find('.kc-file-remove').removeAttr('disabled');
                            if (keys.length === 0 || $.inArray(key, keys) !== -1) {
                                self._setPreviewError($thumb, true);
                                if (!self.retryErrorUploads) {
                                    $thumb.find('.kc-file-upload').hide();
                                    self.fileManager.remove($thumb);
                                }
                            } else {
                                $thumb.find('.kc-file-upload').hide();
            ...
            fnError = function (jqXHR, textStatus, errorThrown) {
                ...
                self._getThumbs(' .kc-file-upload').removeAttr('disabled');
                self._getThumbs(' .kc-file-delete').removeAttr('disabled');
                self._setProgress(101, self.$progress, self.msgAjaxProgressError.replace('{operation}', op));
            };
            var ctr = 0;
        ...
        _initFileActions: function () {
            ...
            self.getFrames(' .kc-file-remove').each(function () {
            ...
            self.getFrames(' .kc-file-upload').each(function () {
                var $el = $(this);
                self._handler($el, 'click', function () {
        ...
        _initPreviewActions: function () {
            var self = this, $preview = self.$preview, deleteExtraData = self.deleteExtraData || {},
                btnRemove = $h.FRAMES + ' .kc-file-remove', settings = self.fileActionSettings,
        ...
        _generatePreviewTemplate: function (
            ...
            prevContent = getContent((forcePrevIcon ? 'other' : cat), data, false, 'kc-preview-thumb');
        ...
        _setThumbStatus: function ($thumb, status) {
            ...
            if (status === 'Error' && !self.retryErrorUploads) {
                $thumb.find('.kc-file-upload').attr('disabled', true);
            }
        },
        ...
        _refreshUploadButton: function ($thumb) {
            var self = this, $btn = $thumb.find('.kc-file-upload'), cfg = self.fileActionSettings,
        ...
        setImageOrientation: function ($img, $zoomImg, value, $thumb) {
            ...
            if (zoomOnly) {
                ...
                $div = $(document.createElement('div')).appendTo($thumb.find('.kc-file-content'));
            ...
            $img.off(ev).on(ev, function () {
                if (isHidden) {
                    ...
                    $thumb.find('.kc-file-content').css('visibility', 'hidden');
                }
                var img = $img[0], zoomImg = $zoomImg && $zoomImg.length ? $zoomImg[0] : null,
                    h = img.offsetHeight, w = img.offsetWidth, r = $h.getRotation(value);
                if (isHidden) {
                    $thumb.find('.kc-file-content').css('visibility', 'visible');
        ...
        _createContainer: function () {
            var self = this, attribs = {'class': 'file-input file-input-new' + (self.rtl ? ' kc-rtl' : '')},
        ...
        _renderMain: function () {
            var self = this,
                ...
                caption = self.captionTemplate.setTokens({'class': css + ' kc-fileinput-caption'});
        ...
        _renderThumbProgress: function () {
            var self = this;
            return '<div class="file-thumb-progress kc-hidden">' +
            ...
            var self = this, reader = self.reader, $container = self.$previewContainer,
                ...
                throwError = function (msg, file, previewId, index, fileId) {
                    ...
                    if (self.removeFromPreviewOnError && $thumb.length) {
                        ...
                        $thumb.find('.kc-file-upload').remove();
                    }
                    self.isPersistentError = true;
        ...
        pause: function () {
            ...
            if (self.showPreview) {
                self._getThumbs().each(function () {
                    ...
                    if (!self._getThumbFile($thumb)) {
                        $thumb.find('.kc-file-remove').removeClass('disabled').removeAttr('disabled');
                    }
                });
            }
        ...
        cancel: function () {
            ...
            self._getThumbs().each(function () {
                ...
                $prog.hide();
                if (!self._getThumbFile($thumb)) {
                    $thumb.find('.kc-file-upload').removeClass('disabled').removeAttr('disabled');
                    $thumb.find('.kc-file-remove').removeClass('disabled').removeAttr('disabled');
                }
                self.unlock();
            });
        ...
        disable: function () {
            ...
            $container.find('.kc-fileinput-caption').addClass('file-caption-disabled');
        ...
        enable: function () {
            ...
            $container.removeClass('is-locked');
            $container.find('.kc-fileinput-caption').removeClass('file-caption-disabled');
            $container.find('.fileinput-remove, .fileinput-upload, .file-preview-frame button')
                .removeAttr('disabled');
    ...
    var IFRAME_ATTRIBS = 'class="kc-preview-data file-preview-pdf" src="{renderer}?file={data}" {style}';
    ...
    $.fn.fileinput.defaults = {
        ...
        previewZoomButtonClasses: {
            ...
            toggleheader: 'btn btn-sm btn-kc btn-default btn-outline-secondary',
            fullscreen: 'btn btn-sm btn-kc btn-default btn-outline-secondary',
            borderless: 'btn btn-sm btn-kc btn-default btn-outline-secondary',
            close: 'btn btn-sm btn-kc btn-default btn-outline-secondary'
        },
        previewTemplates: {},
        previewContentTemplates: {},
        ...
        errorCloseButton: $h.closeButton('kc-error-close'),
        slugCallback: null,
